...
| UCA channel mapping | A/.4 RX1 -> 0, A/.4 RX2 -> 1, B/.5 RX2 -> 2, B/.5 RX1 -> 3 | Required LibreSDR mapping, physically specified by the setup diagram | The graph permutes the two B/.5 outputs so the existing bottom-right, bottom-left, top-left, top-right wiring becomes logical elements 0, 1, 2, 3 without moving cables. |
...
| `run_DoA_transmitter.grc` | Required OTA transmitter configuration | B210 serial selection, 903 MHz center frequency, and 5 kHz pilot are hardware/calibration changes. The user's current uncommitted 89 dB TX gain is preserved and is not treated as an audited port change. |
